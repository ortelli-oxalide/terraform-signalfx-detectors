...
resource "signalfx_detector" "deadlocks_count" {
  ...
  program_text = <<-EOF
        base_filter = filter('resource_type', 'Microsoft.Sql/servers/databases') and filter('primary_aggregation_type', 'true')
        signal = data('deadlock', filter=base_filter and ${module.filter-tags.filter_custom})${var.deadlocks_count_aggregation_function}.publish('signal')
        detect(when(signal > threshold(${var.deadlocks_count_threshold_critical}), lasting="${var.deadlocks_count_timer}")).publish('CRIT')
    EOF
