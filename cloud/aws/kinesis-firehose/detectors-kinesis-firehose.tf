resource "signalfx_detector" "heartbeat" {
  ...
  program_text = <<-EOF
    ...
    signal = data('ResourceCount', filter=filter('stat', 'mean') and filter('namespace', 'AWS/Kinesis') and ${module.filter-tags.filter_custom}).mean(by=['StreamName']).publish('signal')
