resource "signalfx_detector" "heartbeat" {
  ...
  program_text = <<-EOF
    ...
    signal = data('TunnelState', filter=filter('stat', 'mean') and filter('namespace', 'AWS/VPN') and ${module.filter-tags.filter_custom}).mean(by=['VpnId']).publish('signal')
    not_reporting.detector(stream=signal, resource_identifier=['VpnId'], duration='${var.heartbeat_timeframe}').publish('CRIT')
EOF
