resource "signalfx_detector" "heartbeat" {
  ...
  program_text = <<-EOF
    ...
    signal = data('RequestCount', filter=filter('stat', 'sum') and filter('namespace', 'AWS/ApplicationELB') and ${module.filter-tags.filter_custom}).sum(by=['LoadBalancer']).publish('signal')
